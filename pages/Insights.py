...
with tab1:
    db_war_contry = st.session_state.db_war_contry.rename(columns={'Entity': 'country', 'Code': 'code', 'Year': 'year', 'Deaths in ongoing conflicts in a country (best estimate) - Conflict_type: all': 'conflict_deaths'}).copy()
    ...
    db_war_type = st.session_state.db_war_type.rename(columns={'Entity': 'country', 'Code': 'code', 'Year': 'year', 'One-sided violence': 'one_sided_violence', 'Non-state': 'non_state', 'Intrastate': 'intra_state', 'Interstate': 'inter_state'}).copy()
